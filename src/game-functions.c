...
static void cleanup(Window* window, Footer* footer, Board* board, Sprite* chinchilla, Sprite** almonds);
...
int start_game() {
	...
	Window* window = create_window(WINDOW_TITLE, WINDOW_DIMENSION, WINDOW_DIMENSION);
	if (window == NULL) {
		printf("%d", get_window_error());
		return 1;
	}

	Footer* footer = create_footer(WINDOW_FOOTER_X_POSITION, WINDOW_FOOTER_Y_POSITION, WINDOW_FOOTER_WIDTH, WINDOW_FOOTER_HEIGHT);;
	if (footer == NULL) {
	...
	Sprite* chinchilla = create_sprite(window->renderer, CHILLA_IMG, CHINCHILLA_WIDTH, CHINCHILLA_HEIGHT);
	if (chinchilla == NULL) {
	...
	for (int i = 0; i < ALMONDS_COUNT; i++) {
		almonds[i] = create_sprite(window->renderer, ALMOND_IMG, ALMOND_WIDTH, ALMOND_HEIGHT);
		if (almonds[i] == NULL) {
			printf("%d", get_sprite_error());
